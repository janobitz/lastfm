...
class LastFM(BaseCog):
    ...
    def __init__(self, bot):
        ...
        self.payload = {"api_key": "9fed1c5682fcf9dadbf251781ea8e8e6", "format": "json"}
    ...
    async def _nowplaying(self, ctx: commands.Context, member: discord.Member = None):
        ...
        if "@attr" not in track:
            await ctx.send(_("{}, no estás escuchando nada. 😭").format(user))
            return
        ...
        if track["@attr"]["nowplaying"] == "true":
            ...
            song = [song if len(song) < 100 else song[:100] + "..."][0]
            artist = [artist if len(artist) < 100 else artist[:100] + "..."][0]
            album = [album if len(album) < 100 else album[:100] + "..."][0]

            em = discord.Embed()
            em.set_thumbnail(url=image)
            em.add_field(name=_("**Artista**"), value="[{}]({})".format(artist, artist_url))
            em.add_field(name=_("**Pista**"),value="[{}]({})".format(song, track_url),inline=True,)
            em.add_field(name=_("**Reproducciones**"), value="{}".format(playcount))
            em.add_field(name=_("**Álbum**"), value="[{}]({})".format(album, album_url),inline=True,)
